...
LD_LIBRARY_PATH=$DIR ./bsdiff_host "$prev_apk_path" "$apk_path" /tmp/patch

echo "Pushing patch..."
adb push /tmp/patch /data/local/patch

echo "Applying patch..."
adb shell '/data/local/bspatch /data/local/prev_version.apk /data/local/current_version.apk /data/local/patch'

cp "$apk_path" "$prev_apk_path"

echo "Installing..."
adb shell 'pm install -r /data/local/current_version.apk'


adb shell 'mv /data/local/current_version.apk /data/local/prev_version.apk'
